...
def GDAlgorithm(X, Y, lr, iter, err):
    ...
    while (i < iter and test):
        ...
        # Update error if it is better 
        if (er < er_best):
            er_best = er
            w_best = w
            y_best = Y_new
        # Finish if we have a minimun or min error accepted
        if (er < err or er == err):
            test = False
    return(w_best, er_best, y_best)

def multiGD(B, lr, iter, err):
    """ Return a matrix with the predictions of the ratings of the
    movies that users have not seen yet
    - B: links matrix
    - lr: learn rate
    ...
    for i in range(B.shape[1]):
        ...
        W[i], err, S.T[i] = GDAlgorithm(X = X, Y = Y, lr = lr, iter = iter, err = err)
        some += err
    err = some / B.shape[1]
    return (S)
...
def main():
    ...
    iter = 100
    err = 0.5
    S = multiGD(B, lr = lr, iter = iter, err = err)
    print("ERROR KNN TEST: ", error(S,T))
